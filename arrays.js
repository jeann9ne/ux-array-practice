...
function putItemIntoFridge(fridge2, item) {
    if(fridge2.includes(item) === false) fridge2.push(item);
}
...
function getItemFromNewFridge(newFridge, item) {
  ...
  if (itemIndx > -1) {
    let itemArr = newFridge[itemIndx]; 
    itemArr.splice(0, 1); 
    if (itemArr.length === 0){
        newFridge.splice(itemIndx, 1); 
    }
    return item; 
    return null; 
  }
